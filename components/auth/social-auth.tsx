"use client";

import { useState } from "react";
import { UserRound } from "lucide-react";

import { Button } from "@/components/ui/button";
import { Spinner } from "@/components/ui/spinner";
import { toast } from "@/components/ui/toast";
import { continueAsGuest, signInWithGoogle } from "@/services/auth-service";

/** Inline Google "G" mark — lucide dropped brand icons. */
function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden="true">
      <path
        fill="#4285F4"
        d="M23.49 12.27c0-.79-.07-1.54-.19-2.27H12v4.51h6.47a5.53 5.53 0 0 1-2.4 3.58v3h3.86c2.26-2.09 3.56-5.17 3.56-8.82Z"
      />
      <path
        fill="#34A853"
        d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.86-3c-1.08.72-2.45 1.16-4.07 1.16-3.13 0-5.78-2.11-6.73-4.96H1.29v3.09A12 12 0 0 0 12 24Z"
      />
      <path
        fill="#FBBC05"
        d="M5.27 14.29a7.2 7.2 0 0 1 0-4.58V6.62H1.29a12 12 0 0 0 0 10.76l3.98-3.09Z"
      />
      <path
        fill="#EA4335"
        d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42A11.97 11.97 0 0 0 12 0 12 12 0 0 0 1.29 6.62l3.98 3.09C6.22 6.86 8.87 4.75 12 4.75Z"
      />
    </svg>
  );
}

type LoadingState = "google" | "guest" | null;

/** Google OAuth + Continue-as-Guest actions, shown above the email form. */
export function SocialAuth() {
  const [loading, setLoading] = useState<LoadingState>(null);

  async function handleGoogle() {
    setLoading("google");
    const { url } = await signInWithGoogle();
    if (url) {
      window.location.href = url;
      return;
    }
    setLoading(null);
    toast.add({
      title: "Google sign in unavailable",
      description: "Please try again or sign in with your email instead.",
      type: "error",
    });
  }

  async function handleGuest() {
    setLoading("guest");
    const result = await continueAsGuest();
    setLoading(null);

    if (result.success) {
      toast.add({
        title: "Welcome, guest!",
        description: "You can explore the app before creating an account.",
        type: "success",
      });
      window.location.href = "/dashboard";
      return;
    }

    toast.add({
      title: "Could not continue as guest",
      description: result.errors?.form ?? "Please try again or sign in instead.",
      type: "error",
    });
  }

  return (
    <div className="flex flex-col gap-3">
      <Button
        type="button"
        variant="outline"
        size="lg"
        className="w-full"
        onClick={handleGoogle}
        disabled={loading !== null}
      >
        {loading === "google" ? <Spinner /> : <GoogleIcon className="size-4" />}
        Continue with Google
      </Button>
      <Button
        type="button"
        variant="ghost"
        size="sm"
        className="w-full text-muted-foreground"
        onClick={handleGuest}
        disabled={loading !== null}
      >
        {loading === "guest" ? <Spinner /> : <UserRound />}
        Continue as Guest
      </Button>
    </div>
  );
}
